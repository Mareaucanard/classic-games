...
class_name FroggerHighwayLane

signal player_hit_by_vehicle

@export var vehicle_scene: PackedScene = preload("res://Games/Frogger/Actors/Scenes/vehicle.tscn")
...
func _ready():
	for i in vehicle_count:
		var vehicle = vehicle_scene.instantiate() as FroggerVehicle
